...
smartmon/
  ...
  registry.py       load devices.json (or the demo fleet) and wire up backends
  ...
  server.py         FastAPI: the JSON control API + serves the dashboard (production)
...
devices.example.json  copy to devices.json for real hardware (git-ignored: holds keys)
```
...
With no `devices.json` present it serves the demo fleet; drop one in (below) to drive real hardware.

## Configuring real devices

Create `devices.json` next to the app (copy `devices.example.json`). It's **git-ignored** because it
holds each device's Tuya **local key**.
...
Only `ip`, `device_id`, and `local_key` are required per Tuya device. `dps` overrides the DP numbers
and `options` the scaling (`bright_scale`, `power_divisor`, `temp_divisor`, `mode_map`) when a device
deviates from the type defaults in `backends/tuya.py`.

### Getting a device's local key

Local control is AES-encrypted with each device's 16-char **local key**. Extract them once — same
procedure SolarPi documents for the mini-split:

```
pip install tinytuya
python -m tinytuya wizard      # links your Smart Life / Tuya account, prints id + local key per device
python -m tinytuya scan        # finds each device's LAN IP
```

The wizard's live snapshot is the moment to confirm DP numbers and scales against the defaults (e.g.
...
> can appear here as a `climate` device (see `devices.example.json`). Tuya v3.3's LAN protocol allows
...
.venv/bin/pip install tinytuya       # for real Tuya devices
# create ~/smartmon/devices.json from devices.example.json
```
...
Phase 1 (this cut) is the device model, Tuya-local + demo backends, the control API, and the
dashboard. Deliberately **not** built yet (so nothing pretends to work that doesn't):
